...
### 2025-12-10: Callback звонки не записываются в БД

**Проблема**: При callback звонки записывались в таблицу `calls`, хотя не должны были.

**Решение**: Добавлена проверка `command_id` во всех методах обработки webhook'ов. Звонки с `command_id` начинающимся с `callback_` теперь игнорируются и не записываются в БД.

**Детали**: См. `CALLBACK_FIX.md`
